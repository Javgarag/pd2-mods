...
After you've created your animation, it's time to make it available. Download the mod template (additional file in the ModWorkShop page for this download) and revise the following:
...
* Inside `scriptdata/animation_states.xml`, depending on your animation, proceed differently:
	1. If your animation **IS INSTANT**, add it to *SECTION  #1*, and, in *SECTION #6*, change the redirect to `fps/offhand/interact/{ANIMATION_NAME}`. Remove sections *#2, #3, #4, and #5.* In *SECTION #6*, you can add any additional blending/keys you require.
	2. If your animation **IS OF SEPARATE PARTS**, remove *SECTION #1 and SECTION #6*. For the rest of the sections:
