--[[
    ...
    ECM_Timer_v2 Menu specific code made by Dom
    List:
    Items in line
...
ECM_Timer_v2Menu = ECM_Timer_v2Menu or class()
function ECM_Timer_v2Menu:init()
   local aspect_ratio = RenderSettings.resolution.x / RenderSettings.resolution.y
	...
	if ECM_Timer_v2Menu.Warning == 1 then
		self:CreateChangeWarning()
	end

    self:GetMenuFromJson(ECM_Timer_v2._menu_path .. "MainMenu.json", ECM_Timer_v2._data)

    self:OpenMenu("ECM_Timer_v2_menu")
end

function ECM_Timer_v2Menu:CallCallback(item, params)
...
function ECM_Timer_v2Menu:update(t, dt)
...
function ECM_Timer_v2Menu:SetAxisTimer(axis, delay, input_delay, input)
...
function ECM_Timer_v2Menu:Open()
    self._enabled = true
    ...
    if not self._controller then
        self._controller = managers.controller:create_controller("ECM_Timer_v2Menu", nil, false)
...
function ECM_Timer_v2Menu:Close()
    self._enabled = false
...
-- Mouse Functions
function ECM_Timer_v2Menu:mouse_move(o, x, y)
...
function ECM_Timer_v2Menu:mouse_press(o, button, x, y)
    x, y = self:_convert_mouse_pos(x, y)
    if button == Idstring("0") then
        ...
        elseif self._input then
            if self._input.panel:inside(x, y) then
                --ECM_Timer_v2:Log(Activate input")
            else
                self:CloseInput()
            end
...
function ECM_Timer_v2Menu:mouse_release(o, button, x, y)
    if button == Idstring("0") then
        if self._slider then
...
-- Item interaction
function ECM_Timer_v2Menu:Confirm()
...
function ECM_Timer_v2Menu:MenuDown()
    if not self._enabled then
        return
    end
    if self._open_choice_dialog then
...
function ECM_Timer_v2Menu:MenuUp()
    if not self._enabled then
        return
    end
    if self._open_choice_dialog then
...
function ECM_Timer_v2Menu:MenuLeftRight(change)
    if not self._enabled then
        return
    end
...
function ECM_Timer_v2Menu:ActivateItem(item, x)
...
function ECM_Timer_v2Menu:CreateChangeWarning()
	if managers.hud and not self._panel:child("changed_warning") then
		ECM_Timer_v2Menu.Warning = 1
		local changed_warning = self._panel:text({
			...
			color = Color.red,
			align = "right",
			text = managers.localization:text("ECM_Timer_v2_warning_desc")
		})
...
function ECM_Timer_v2Menu:SetMenuItemsEnabled(menu, parent_item_id, enabled)
...
function ECM_Timer_v2Menu:GetMenu(menu)
    return self._menus[menu]
end

function ECM_Timer_v2Menu:AnimateItemEnabled(item, enabled)
...
function ECM_Timer_v2Menu:HighlightItem(item)
...
function ECM_Timer_v2Menu:UnhighlightItem(item)
    if item.focus_changed_callback then
...
function ECM_Timer_v2Menu:SetLegends(accept, reset, step)
...
function ECM_Timer_v2Menu:Cancel()
    if self._open_choice_dialog then
        self:CloseMultipleChoicePanel()
    elseif self._open_color_dialog then
        self:CloseColorMenu(false)
...
function ECM_Timer_v2Menu:SetItem(item, value, menu)
...
function ECM_Timer_v2Menu:GetMenuFromJson(path, ...)
    ...
    if file then
        ...
        if content.title == "ECM_Timer_v2_title" then
            menu_title = menu_title .. " " .. ECM_Timer_v2.ModVersion
        end
...
function ECM_Timer_v2Menu:CreateItem(item, items, menu_id, ...)
        ...
        if type(parents) == "string" then
            for _, pitem in pairs(items) do
                if pitem.id == parents then
                    enabled = ECM_Timer_v2._data[pitem.value]
                    break
                end
            end
        elseif type(parents) == "table" then
            ...
            for _, parent in ipairs(parents) do
                for _, pitem in pairs(items) do
                    if pitem.id == parent then
                        final = final and ECM_Timer_v2._data[pitem.value]
                        break
                    end
                end
            end
    ...
    value = ECM_Timer_v2._data[item.value]
    ...
    elseif item_type == "color_select" then
        value = ECM_Timer_v2:GetColor(item.value)
...
function ECM_Timer_v2Menu:CreateOneLineItems(item, items, menu_id, ...)
    local offset = {
        ["label"] = 5,
        ["button"] = 10,
        ["toggle"] = 34,
...
function ECM_Timer_v2Menu:CreateMenu(params)
...
function ECM_Timer_v2Menu:OpenMenu(menu, close)
...
function ECM_Timer_v2Menu:AddItemToMenu(menu_id, item, total_size)
...
function ECM_Timer_v2Menu:GetLastPosInMenu(menu_id)
    return self._menus[menu_id].len or 50
end

--Label Items
function ECM_Timer_v2Menu:CreateLabel(params)
...
--Divider Items
function ECM_Timer_v2Menu:CreateDivider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Button Items
function ECM_Timer_v2Menu:CreateButton(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
--Toggle Items
function ECM_Timer_v2Menu:CreateToggle(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    ...
    local check_bg = toggle_panel:bitmap({
        name = "check_bg",
        texture = "guis/textures/pd2_mod_ECM_Timer_v2/check_bg",
        x = 2,
        y = 2,
        w = 22,
        h = 21,
    ...
    local check = toggle_panel:bitmap({
        name = "check",
        texture = "guis/textures/pd2_mod_ECM_Timer_v2/check",
        x = 2,
        y = 2,
...
--Slider Items
function ECM_Timer_v2Menu:CreateSlider(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function ECM_Timer_v2Menu:SetSlider(item, x, add)
...
function ECM_Timer_v2Menu:SetInput(item)
    if self._input then
        self:CloseInput()
    end
    self._input = item
    --ECM_Timer_v2:Log("Input")
...
function ECM_Timer_v2Menu:key_press(item, o, k)
...
function ECM_Timer_v2Menu:enter_text(item, o, s)
    local text = item.value
    local m = item.limit
    local n = utf8.len(text)
    s = utf8.sub(s, 1, m - n)
...
function ECM_Timer_v2Menu:key_release(item, o, k)
    if ECM_Timer_v2:IsOr(k, Idstring("esc"), Idstring("enter")) then
        self:CloseInput()
    end
end

function ECM_Timer_v2Menu:CloseInput()
    if self._input then
...
function ECM_Timer_v2Menu:CreateMultipleChoice(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
...
function ECM_Timer_v2Menu:OpenMultipleChoicePanel(item)
...
function ECM_Timer_v2Menu:CloseMultipleChoicePanel()
...
function ECM_Timer_v2Menu:CreateInput(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    ...
    input_panel:bitmap({
        texture = "guis/textures/pd2_mod_ECM_Timer_v2/hud_corner",
        name = "left_top",
        visible = true,
        layer = 0,
        y = 0,
...
-- Custom Color Items
function ECM_Timer_v2Menu:CreateColorSelect(params)
    local menu_panel = self._options_panel:child("menu_"..tostring(params.menu_id))
    if not menu_panel or not self._menus[params.menu_id] then
        return
    end
    local texture = params.texture or nil
    local texture_extension = "_ECM_Timer_v2"
...
function ECM_Timer_v2Menu:OpenColorMenu(item)
    ...
    red_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("ECM_Timer_v2_r"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    green_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("ECM_Timer_v2_g"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    blue_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("ECM_Timer_v2_b"),
        x = 85,
        w = red_panel:w() - 90,
        h = 25,
        align = "right",
    ...
    accept_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("ECM_Timer_v2_ok"),
        x = 5,
        w = red_panel:w() - 10,
        h = 25,
        align = "right",
    ...
    reset_panel:text({
        ...
        font_size = 18,
        font = tweak_data.menu.pd2_small_font,
        text = managers.localization:text("ECM_Timer_v2_color_reset"),
        x = 5,
        w = red_panel:w() - 10,
        h = 25,
        align = "right",
...
function ECM_Timer_v2Menu:SetColorSlider(item, x, type, add)
...
function ECM_Timer_v2Menu:CloseColorMenu()
...
function ECM_Timer_v2Menu:ResetColorMenu()
...
function ECM_Timer_v2Menu:SetOption(value, option)
    ECM_Timer_v2._data[option] = value
end

function ECM_Timer_v2Menu:SetColorOption(color, option)
    local c = ECM_Timer_v2._data[option]
    c.r = color.red
    c.g = color.green
    c.b = color.blue
end