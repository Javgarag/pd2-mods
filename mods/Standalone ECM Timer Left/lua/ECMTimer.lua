if RequiredScript == "lib/managers/hudmanagerpd2" then
    ...
    function HUDECMCounter:init(hud)
	    ...
	    self._ecm_panel:set_top(70)
        self._ecm_panel:set_right(self._hud_panel:w() - 1178)
		...
		local pager_icon = self._ecm_panel:bitmap({
		    ...
		    valign = "top",
			visible = false,
			color = ECM_Timer_v2:GetColor("ECMUpgradeIcon"),
		    layer = 2,
		    w = ecm_box:w() / 2,
		    h = ecm_box:h() / 2
	    })
	...
	function HUDECMCounter:update_icons(jam_pagers)
        ...
        pager_icon:set_visible(jam_pagers and ECM_Timer_v2:GetOption("pager_jam"))
		self._pager_block = jam_pagers
    end

	--Init
	Hooks:PostHook(HUDManager, "_setup_player_info_hud_pd2", "ECM_Timer_v2_setup_player_info_hud_pd2", function(self)
		self._hud_ecm_counter = HUDECMCounter:new(managers.hud:script(PlayerBase.PLAYER_INFO_HUD_PD2))
	end)
	
	--Update ECM timer
	Hooks:PostHook(HUDManager, "update", "ECM_Timer_v2_HUDManager_update", function(self)
...
elseif RequiredScript == "lib/units/equipment/ecm_jammer/ecmjammerbase" then
	...
	Hooks:PostHook(ECMJammerBase, "setup", "ECM_Timer_v2_ECMJammerBase_setup", function(self, battery_life_upgrade_lvl, ...)
	...
	Hooks:PostHook(ECMJammerBase, "sync_setup", "ECM_Timer_v2_ECMJammerBase_sync_setup", function(self, upgrade_lvl, ...)
		local new_end_time = TimerManager:game():time() + self:battery_life()
		if new_end_time > managers.hud._hud_ecm_counter._end_time then
	...
	Hooks:PostHook(ECMJammerBase, "update", "ECM_Timer_v2_ECMJammerBase_update", function(self, unit, t, ...)
...
elseif RequiredScript == "lib/units/beings/player/playerinventory" then
	Hooks:PostHook(PlayerInventory, "_start_jammer_effect", "ECM_Timer_v2_PlayerInventory__start_jammer_effect", function(self, end_time)
		local new_end_time = end_time or TimerManager:game():time() + self:get_jammer_time()
		if ECM_Timer_v2:GetOption("pocket_ecm") and new_end_time > managers.hud._hud_ecm_counter._end_time or managers.hud._hud_ecm_counter._pager_block == false then
			managers.hud._hud_ecm_counter._end_time = new_end_time
